...
同一首歌 `online:netease:4875626`：

```
09:54:22,004 → 302      09:54:23,572 → 302
09:54:22,850 → 302      09:54:23,796 → 302
09:54:23,047 → 302      09:54:23,984 → 302
09:54:23,284 → 302      09:54:24,210 → 302
                        09:54:24,485 → 302      ← 2.5 秒 9 次，然后就被切走
```

而同一时刻另一首 `online:netease:413829859`：**全程只有 1 次请求**（09:54:28），之后 **31 秒再无动静**。

后者才是 302 该有的样子——拿一次直链，然后自己去 CDN 拉很久。前者反复回来要同一首歌，只说明一件事：**客户端没能从那个 CDN 地址取到音频**（地址被拒、连接被断、或客户端不跟重定向），重试到放弃就跳歌。

### 难处：这一程我们本来是瞎的

客户端↔CDN 发生在我们之外。2.9.27 那套「失败留证」只覆盖到**取直链**为止，之后是盲区——所以诊断页显示「暂无记录」，而歌照样被切。留证做得再细，也看不见自己没参与的那一半。

### 本次改动

**1. 按行为兜底（核心）**

同一首歌 **15 秒内被请求 3 次以上** → 判定这次直连没成功，从下一次起**改走 NAS 中转**。

判据是客户端的行为，不是我们的猜测：我们看不见 CDN 那一程，但看得见它是不是在反复回来。回退走的是 2.9.28 之前一直在用的老路，行为完全已知——**出问题最多退化到旧版本，不会更糟**。

这样 302 从「非此即彼的开关」变成「能用就直连、不能用自己退回中转」。

**2. 补回播放起步留证**

2.9.28 的 302 分支直接 `return`，`play-start` 一行都没落，于是诊断页出现**「实际播放 0 首」而预热统计却记着 6 首**的自相矛盾，「每次播放请求数」这个能看出客户端是否稳定取流的指标也一并瞎了。现在 302 也记 `play-start`。

同时带上直链的 **`scheme://host`**（不记路径——那是令牌）。scheme 是关键线索：若真机拿到的是 `http://` 明文直链，部分客户端默认禁止明文流量，「302 出去却取不到」就有了明确解释。

**3. 诊断页**「音频走哪条路」新增 **「直连失败回退 N 首」** 与读法说明；失败留证新增 `redirect-storm` 阶段（每首歌只告警一次，避免用新问题换旧问题）。

> 若「直连失败回退」长期不为 0，说明 302 这条路不适合你的客户端——直接关掉开关更省事。
...
### 1. 非局域网 302 直连 CDN（核心）

以前不管在哪，音频路径都是 **CDN → NAS → 手机**。局域网时这没问题（两端都在内网，带宽管够）；数据网络/异地远程时，音频要**两次穿过 NAS 的上行**（进一次、出一次），而家里上行通常只有下行的一小部分——这才是「流量下卡」的结构性原因，跟音质、延迟都不是一回事。

现在：**明确判为远程/流量**时直接回 `302`，让手机自己去 CDN 取，音频一次都不经过 NAS。

| 场景 | 音频路径 | 边播边存 |
|---|---|---|
| 局域网（家里 WiFi / 内网） | CDN → NAS → 手机（照旧） | **保留** |
| 非局域网（流量 / 异地公网） | CDN → 手机（302 直连） | 关掉（音频不经 NAS，本来也存不下来） |
| 判不出（unknown） | CDN → NAS → 手机（照旧） | 保留 |

> 判不出的时候**不贸然绕开**。「不知道客户端在哪」就保持原来的行为，是保守做法——万一其实在家里，绕开只会平白丢掉边播边存。

「边播边存」在非局域网下不是被"关掉"，而是**物理上做不到**：音频不经过 NAS，NAS 就没有字节可存。留着那条 tee 分支只会白占一个缓存位。

新增开关 **「非局域网直连 CDN（不经 NAS 中转）」**（默认开），一键可关回全部经 NAS。诊断页新增 **「音频走哪条路」** 区块，分别给出 302 直连 / NAS 中转 / 本地文件的次数。

### 2. 取链超时（默认 5 秒）

以前取直链可能一直挂着，播放器 7~8 秒等不到就自己跳歌，而我们还在原地傻等——用户看到的是「点了没反应 → 自动跳过 → 手动切几首 → 彻底放不出声」，日志里却什么都没有。

现在：取直链 + 元数据的并发超过设定值就直接放弃本次取链，回给飞牛让它**切下一首**，不再空耗到你手动切。

- 默认 **5 秒**，管理页可调 **1~30 秒**
- 生效值 = `min(你设的值, 上限 30 秒)`，且与代理自身超时再取小，不会比 30 秒还长
- **全局生效**（局域网、非局域网一视同仁）——局域网内很少触发，真触发了说明 musicbox 卡住了，早点放弃同样是对的

### 3. 管理页清理

按你说的，以下项从页面上移除（相关代码一并清掉，默认值改为关/废除，不影响其他功能）：

| 移除项 | 说明 |
|---|---|
| VIP 剩余 | 从未实现，一直显示不出来 |
| 收藏同步回网易云 | 从未实现，默认值一并改为关闭 |
| 固定音质档 | 取消；音质策略默认改为「按局域网」 |
| VIP 到期提醒提前量（天） | 连同 VIP 提醒一起取消 |
| 飞牛授权目录（本地曲库访问权）卡片 | 卡片与相关脚本移除 |
| 最下边的日志卡片 | 卡片移除；**诊断页里的日志保留** |

**音质**这一块只保留两档，且档位名与网易云完全一致（高→低）：

- 局域网（家里 WiFi / 内网）听什么
- 非局域网（流量 / 异地远程）听什么

档位：`超清母带 jymaster` / `Hi-Res hires` / `无损 lossless` / `极高 320k exhigh` / `较高 higher` / `标准 standard`

> 页面/选项层的删除按「能隐藏就隐藏、尽量小改代码」来做的，避免动到底层逻辑引出新 bug。

## [2.9.27] - 2026-09-16

**「等 7~8 秒然后自动跳过」——这次先把黑盒拆开：以前跳完什么都不留，现在每一步都留痕。**

### 为什么查不到原因

真机出现过「流量下播推荐歌单，一首歌等 7~8 秒后自动跳过，手动切几首后彻底放不出声，飞牛音乐闪退」。翻日志时撞上一个更难堪的事实：**播放器跳过一首歌，我们的日志里一行都没有**。`_online_unavailable()` 直接返回 404 就结束了，既不知道失败在哪一步，也不知道耗时多久，事后只能靠猜。

### 本次改动

1. **播放失败留证**（核心）。`_online_unavailable()` 的每个出口都落一行日志并记进环形缓冲，按**阶段**分类——它决定了该往哪个方向查：

   | 阶段 | 含义 | 该查什么 |
   |---|---|---|
   | `resolve` | 连直链都没拿到 | musicbox 慢/超时、网易云登录态掉了、接口空返回 |
   | `open-cdn` | 直链拿到了但取流被拒 | 直链过期、403、CDN 抽风 |
   | `slow-start` | 取链本身就超过 5s | 播放器 7~8s 跳歌时这一段已占掉大半 |

   诊断页新增「最近播不出来」区块（时间 / 阶段 / 曲目 / 耗时 / 原因），另开 `/_ext/failures` 端点。CDN 返回 4xx/5xx 的分支以前也是静默的，现在一并记状态码与 content-type。

2. **慢起步告警**。gather（直链 + 元数据）耗时超过 5s 单独告警：`play slow-start <guid>: 取链+元数据 耗时 Xms`。以前「未预热 100ms」这种均值很好看，但均值掩盖了尾部——真正让用户跳过的是那几次几秒的。

3. **HLS 慢分片留证**。诊断里「分片转发耗时 最大 30106.9ms / 平均 193.3ms」是本次最硬的异常（差 155 倍），可它只有一个孤零零的 max：看得到「有一片 30 秒」，却不知道是哪首歌、一共几次。现在超过阈值（默认 5s，`FNMUSIC_HLS_SLOW_SEG_MS`）的分片单独计数并告警，同时记下最慢那片属于哪首歌。播放器 7~8 秒等不到就跳歌，而我们这边的转发会继续挂着——**用户感知的「跳过」和日志里的「超时」是同一件事的两半**。

4. **预热 lookahead 上限 5 → 3**。真机 `.env` 里的 `LOOKAHEAD=5`（升级不覆盖用户值）配上 `max_queue=2`，结果是 6 次被队列拒、11 次撞上「已预热过」——预热全在空转，却实打实占着单进程的 musicbox，被拖慢的恰恰是正在播的那首。预热是纯优化，砍到 3 首不会让任何功能退化；超出部分诊断页会明确标注「已收敛 N → 3」。

> 本次**不改播放行为**，只把失败从「静默」变成「留证」，外加收敛一处空转的预热。装完后若再遇到跳歌，直接把诊断页「最近播不出来」那一段贴出来即可定位，不用再翻几 MB 的 `proxy.log`。

## [2.9.26] - 2026-09-15

**修「应用显示未启用、要手动点一次」——我们把瞬时故障上报成了停机。**

### 规范怎么说

飞牛开发文档 [`cmd/main`](https://developer.fnnas.com/docs/core-concepts/framework/) 的约定：

| 退出码 | 含义 |
|---|---|
| `0` | 成功，或在 `status` 中表示**正在运行** |
| `1` | 失败 |
| `3` | 在 `status` 中表示**未运行** |

应用中心据此显示「已启用 / 未启用」——**而这个状态不会因为应用自己恢复了就自动翻回来**，
用户必须手动再点一次「启用」。

### 我们错在哪

以前 `status.sh` 回答的是「**这一刻 socket 有没有接管成功**」：

```
官方 trim-music 重启 → 重绑 trim_music.socket 抢走接管（代理进程毫发无损）
        ↓
status.sh 报 exit 3
        ↓
飞牛把应用置为「未启用」
        ↓
看门狗几十秒内自愈了，但状态不会自动翻回来 → 用户手动点「启用」
```

**等于一边用看门狗自愈、一边告诉系统自己没在运行，两个机制打架。**

其实我们早就知道这个道理——管理页「保存配置即闪退」用的就是同一套思路
（`lib_restart_in_progress` → 仍报 running）。**只是那个妥协只覆盖了配置重启窗口，
漏了看门狗恢复窗口。**

### 现在怎么判

`status` 回答正确的问题：**「这个应用是否仍处于启用状态」**。

| 情形 | 判定 | 理由 |
|---|---|---|
| 代理进程在 + 接管正常 | `running` | 一切正常 |
| 代理进程在 + 接管丢失 + **看门狗在管** | `running` | 仍启用，正在自愈（日志写明「接管丢失 Ns」） |
| 代理进程在 + 接管丢失 + 看门狗不在 | `not running` | 没人会来救了 |
| 丢失超过 **10 分钟**宽限仍未恢复 | `not running` | 看门狗真救不回来，该人工介入 |
| 代理进程不在 | `not running` | 真的没起来 |

### 顺带修掉看门狗的误杀

接管探针 `lib_probe_proxy` 有 3s 超时，而 `/_ext/healthz` 内部要连上游 + musicbox 三次。
上游偶尔抖一下 → 超时 → 误判「接管丢失」→ 走进 `start.sh` 的「先停后起」分支
**把健康的代理杀掉**，代价是 1~2 分钟服务中断。

真机 01:51:43 那次就是误报：报丢失后 3 秒（正好一个 curl 周期）就「接管正常」了。

现在命中这一分支时**隔 5 秒复测一次**，确认仍失败才动手。进程不存在的分支不复测——
那没有歧义，越早恢复越好。

### ⚠️ 本次升级后仍需要手动点一次「启用」

那是升级**之前**就落下的状态，新版本管不了。装上新版之后不会再误报。

## [2.9.25] - 2026-09-14

**播放提速三件事：复用连接、预热让位、修 warm 口径。**

上版诊断给出两个反直觉数字，正是本次动机。

### ① 连接复用 —— 「每次播放请求数 14.62」暴露的浪费

一次播放客户端要发十几个 Range 请求，而取流时**每次都新建一个 httpx 客户端、用完就拆**，
等于每个 Range 重新做一次 TCP + TLS 握手（TLS 通常 2 个 RTT）。
...
这笔开销跟音质、带宽**全无关系**。现在进程内共享一个取流客户端，由连接池按域名复用
（上限 20 连接）。流没读完就关的请求，httpx 会丢弃那条连接而不是交给下一个请求，不会串数据。

### ② 预热让位 —— 不再跟正在播的抢 musicbox

musicbox 是单进程。用户连续切歌时**每一首**都会追加 N 个预热任务，队列越堆越长、
消化不过来，最后被拖慢的恰恰是「正在播的那一首」。

- 新增总闸门 `FNMUSIC_PREFETCH_MAX_QUEUE`（默认 **2**）：同时在飞的预热满了就放弃新预热
- `FNMUSIC_PREFETCH_LOOKAHEAD` 默认 3 → **2**

预热是锦上添花，跟播放冲突时必须让位。⚠️ 已在 `.env` 里手动设过 `LOOKAHEAD=5` 的不受影响，
请到管理页调到 2~3。

### ③ 修 warm 口径（本次最关键）

`warm` 以前标的是「**曾经预热过这首歌**」，窗口写死 **3600s**；而预热成果的真实寿命是
直链缓存 TTL（**600s**）——**标记比成果长寿 6 倍**。

于是那些一次缓存都没命中、照付全额 musicbox 往返的播放，也被记进了 warm。均值失真到给出：

```
未预热 114.2ms → 命中预热 332ms      ← 预热反而慢 3 倍？
```
...
| 项 | 改前 | 改后 |
|---|---|---|
| warm 含义 | 曾经预热过这首歌 | **这次真的省掉了 musicbox 往返**（直链缓存是否命中） |
| 有效期 | 写死 3600s | 与直链缓存 TTL 对齐 |
| 「撞上预热正在跑」 | 混在 cold 里 | **单独计数**（它不仅没省，还要跟预热抢 musicbox） |

诊断页新增「队列上限」「成果有效期」「撞上预热进行中」三行。

### 验收

| 指标 | 期望 |
|---|---|
| 「取链耗时」那行 | `省下 X ms` 的 **X 必须变正数**（改前是 -217ms 的荒唐值） |
| 「每次播放请求数」 | 应低于 14.62（下降说明之前确实在反复重建连接） |
| 「撞上预热进行中」 | 越接近 0 越好 |

## [2.9.24] - 2026-09-14

**修掉 `play-start` 日志刷屏：一次播放被记成十次。**

看 WiFi 诊断时发现同一首歌 `online:netease:481514905` 在 `20:56:14.607` → `20:56:15.220`
不到 0.6 秒里刷出 **10 行 `play-start`**，全是 `local-first(pre-net,ext=flac,level=jymaster)`、
耗时 11~19ms。它们不是播了十次，而是**同一次播放的十几个 Range 续传请求**
（ijk 起播探测 + 分片续传），每个都完整走一遍 `/track/stream`。

两个后果：

1. 日志里「播了几首」被放大十倍，拿它评估预热命中率必然失真；
2. **本地直出也被一律标成 `cold`** —— 本地文件压根没联网、不吃预热，
   标 `cold` 读起来就是「慢是因为没预热」，而事实恰恰相反（它是最快的那条路）。

本次改动：

| 项 | 说明 |
|---|---|
| 去重 | 同一 guid 在时间窗内只落一行 `play-start`，续传只累加计数。口径与预热统计一致，默认 600s，可调 `FNMUSIC_PLAY_START_DEDUPE_S`（`0` = 不去重） |
| 标记三态 | `local` = 音频走本地文件（`local-first` / `local-daily` / `tee-cache` / `local-fallback`，与预热无关）；`cold` / `warm` 只留给真正走网易云的播放 |
| 旁路日志 | `local-first skip` / `local-first hit` 的重复提示同样折叠（用「窥探」模式，不占用主行去重额度） |
| 诊断页 | 新增「播放起步日志」区块：实际播放首数、折叠掉多少续传、**每次播放请求数** |

「每次播放请求数」怎么读：一次播放本来就要分几个 Range 拉，比值 2~10 都正常；
到几十就说明客户端在反复重开连接——那才是真正的卡顿信号。
